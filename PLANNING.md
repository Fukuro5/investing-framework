...
**Resolved and implemented**: the pure decision-matrix logic lives in `lib/signals/compute-position-signal.ts` (`computePositionSignal`), taking a thesis verdict, an array of metric-rule evaluation statuses, and a position's allocation %/band, and returning the badge plus each intermediate severity for the "why" breakdown. The DB-backed read model, `getActiveFrameworkPositionSignals` (`lib/signals/get-active-framework-position-signals.ts`), computes it for every position assigned to a group in the active framework, in one batch call. **Not persisted to the `Signal` table**: that table's schema (one row per single `GroupRule` evaluation, `status` in `ok`/`warn`/`breach`) doesn't fit a badge combining three inputs into one `trim`/`buyMore`/`sell`/`hold` vocabulary — the badge is computed fresh on every read instead, same as `getActiveFrameworkAllocations`. **Positions with no group assignment get no badge** (shown as unclassified) — there's no group's rules/band to combine with. Surfaces as a new "Signal" column on the dashboard's positions table (`PositionsTable.tsx`/`PositionSignalCell.tsx`) — an expandable `<details>` badge, no client JS needed — rather than on the thesis page. **Missing-input defaults**: a never-checked thesis (`ThesisVerdict` is null) counts as `good` rather than penalizing an unassessed position; a missing allocation band or unresolved allocation % counts as `inBand` (neutral) rather than guessed.
...
Any new v2 tables/fields get added here as they're designed, rather than bolted onto the v1 shape without review. All of the below are now **resolved and implemented** — full detail in §1:
...
**Resolved and implemented (§1 Phase 1)**: `GroupRule` gains a `type` discriminator (`'allocation' | 'metric'`) plus a `scope` field (`'group' | 'position'`, allocation-only) — the type/scope values themselves are hardcoded, but every rule instance stays fully user-managed via CRUD, same as today. `FrameworkGroup.targetAllocationMin`/`Max` are removed and become each group's required `type='allocation', scope='group'` rule; a new optional `type='allocation', scope='position'` rule adds a uniform min/max band applied to every position in the group (not a per-instrument override). Metric rules keep their current shape, and signal-role metric rules (previously hardcoded off in `create-rule.ts`) are un-deferred — `role` is a free, validated input on `createRule`, no longer forced to `'classification'` — since Phase 5's signal engine now evaluates them.
...
- **Phase 5 — Framework rules**: metric catalog + `MetricValue` ingestion, `GroupRule` CRUD, auto-classification engine. Signal badges (trim/buy-more/sell/hold) were deliberately deferred to v2 at this point — despite this line's original wording, no badge computation shipped in v1; see v2 Phase 5 below for where that landed.

### v2 — Signals (full detail in §1) — all five phases shipped
...
- **Phase 5** — Signal engine: Health (worst of thesis/metric severity) × allocation sizing action → final trim/buy-more/sell/hold badge via a decision matrix, plus a per-input "why" breakdown. Group-level allocation anomalies surface separately (already handled by v1's `GroupAllocationSummary`), not folded into position badges. Computed on demand (`lib/signals/`), not persisted; surfaced as a new Signal column on the dashboard. See §1 for details.
...
- **Signal badge is computed on demand, not persisted**: the `Signal` table's schema (one row per single `GroupRule` evaluation) doesn't fit a badge combining three inputs (thesis, metric rules, allocation) into one `trim`/`buyMore`/`sell`/`hold` value, so it stays unused — the badge is recomputed fresh on every dashboard read instead, same treatment as `getActiveFrameworkAllocations` (§1 Phase 5, §3).
- **Signal badge surfaces on the dashboard only**, as a new column on the positions table with an expandable "why" panel — not duplicated onto the thesis page, to keep one place per concern (§1 Phase 5).
